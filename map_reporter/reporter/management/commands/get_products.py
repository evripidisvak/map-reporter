...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        map_products = pd.read_csv(
            current_folder + "/reporter/management/commands/map-products-api.csv"
        )
            ...
            for data in meta_data:
                if data["key"] == "_b_price":
                    ...
                    if data["value"]:
                        ...
                        # This was equal to 1. But we  do not want to change the price in soundstar.gr. We did not try this with 0. Give it a go and good luck.
                        map_price = 0
                    break
            ...
            if row["Skroutz"]:
                if not pd.isnull(row["Skroutz"]):
                    Page.objects.get_or_create(
                        url=row["Skroutz"],
                        product=product,
                        source=Source.objects.get(name="Skroutz"),
                    )
            if row["Electronet"]:
                if not pd.isnull(row["Electronet"]):
                    Page.objects.get_or_create(
                        url=row["Electronet"],
                        product=product,
                        source=Source.objects.get(name="Electronet"),
                    )
            if row["Germanos"]:
                if not pd.isnull(row["Germanos"]):
                    Page.objects.get_or_create(
                        url=row["Germanos"],
                        product=product,
                        source=Source.objects.get(name="Germanos"),
                    )
            # if row['Media Markt']:
            ...
            if row["You"]:
                if not pd.isnull(row["You"]):
                    Page.objects.get_or_create(
                        url=row["You"],
                        product=product,
                        source=Source.objects.get(name="You"),
                    )
            if row["Public"]:
                if not pd.isnull(row["Public"]):
                    Page.objects.get_or_create(
                        url=row["Public"],
                        product=product,
                        source=Source.objects.get(name="Public"),
                    )
            if row["Kotsovolos"]:
                if not pd.isnull(row["Kotsovolos"]):
                    Page.objects.get_or_create(
                        url=row["Kotsovolos"],
                        product=product,
                        source=Source.objects.get(name="Kotsovolos"),
                    )
            if row["Praktiker"]:
                if not pd.isnull(row["Praktiker"]):
                    Page.objects.get_or_create(
                        url=row["Praktiker"],
                        product=product,
                        source=Source.objects.get(name="Praktiker"),
                    )
            if row["Plaisio"]:
                if not pd.isnull(row["Plaisio"]):
                    Page.objects.get_or_create(
                        url=row["Plaisio"],
                        product=product,
                        source=Source.objects.get(name="Plaisio"),
                    )
